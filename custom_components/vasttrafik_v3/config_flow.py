"""Config flow: user (credentials) → start_stop → end_stop → pick_line →
line_options. When an endpoint is given the direction is derived from it;
otherwise all departures on the line are shown.
"""
...
def _lines_from_departures(departures: list[dict]) -> list[dict]:
    """Unique lines from departures, naturally sorted.
...
def _best_direction_for_endpoint(
    ...
    """Return the (direction, direction_gid) of *line_name* best matching
    *end_name*. The v4 departure model has no terminus GID, so the gid slot is
    always "" here — server-side directionGid is resolved separately."""
    ...
    for dep in departures:
        try:
            ...
            if end_lower in direction.lower():
                return direction, ""
            if not best_dir:          # first direction seen, as a fallback
                best_dir = direction
        except Exception as exc:  # noqa: BLE001
...
class VasttrafikConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _fetch_lines_then_show_picker(self) -> dict:
        """Fetch departures from the start stop, narrow to lines serving the
        endpoint when one was given, then show pick_line."""
    ...
    async def async_step_pick_line(self, user_input: dict | None = None) -> dict:
        """Pick a line, derive direction from the endpoint, then line_options."""
        errors: dict = {}
        ...
        if user_input is not None:
            ...
            if not short:
                ...
                try:
                    ...
                    # Resolve terminus GID for server-side directionGid filtering;
                    # falls back to client-side filtering when unresolvable.
        ...
        if not self._available_lines:
            # No departures found — fall back to manual text entry.
    ...
    async def async_step_line_options(self, user_input: dict | None = None) -> dict:
        """Walk-time offset and display name; commits the monitored line."""
